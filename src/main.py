    ...
    *,
    ...
    if isinstance(kalshi_cfg, dict) and kalshi_cfg.get("enabled", False):
        try:
            ...
            if isinstance(theta_cfg, dict) and theta_cfg.get("enabled", False):
                if "kalshi_theta" in enabled:
                    from .engines.kalshi_theta_engine import KalshiThetaEngine
                    theta_engine = KalshiThetaEngine(
                        config=config,
                        kalshi_client=kalshi_read,
                    )
